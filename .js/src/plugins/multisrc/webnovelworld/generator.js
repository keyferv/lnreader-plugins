...
var generateAll = function () {
    return sources_json_1.default.map(function (source) {
        ...
        console.log("[webnovelworld] Generating: ".concat(source.id).concat(' '.repeat(20 - source.id.length), " ").concat(source.filters ? '🔎with filters🔍' : '🚫no filters🚫'));
...
var generator = function generator(source) {
    ...
    var pluginScript = "\n".concat(LightNovelWPTemplate, "\nconst plugin = new WebNovelWorld(").concat(withFilterTypeRefs(JSON.stringify(source)), ");\nexport default plugin;\n    ").trim();
    return {
