...
var generateAll = function () {
    return sources_json_1.default.map(function (source) {
        ...
        console.log("[readwn] Generating: ".concat(source.id).concat(' '.repeat(20 - source.id.length), " ").concat(source.filters ? '🔎with filters🔍' : '🚫no filters🚫'));
        return generator(source);
    });
};
exports.generateAll = generateAll;
var generator = function generator(source) {
    var readwnTemplate = (0, fs_1.readFileSync)((0, path_1.join)(folder, 'template.ts'), {
        encoding: 'utf-8',
    });
    var pluginScript = "\n".concat(readwnTemplate, "\nconst plugin = new ReadwnPlugin(").concat(JSON.stringify(source), ");\nexport default plugin;\n    ").trim();
    return {
        lang: 'english',
