...
function getFilters(name, html) {
    ...
    $('input[name="country_id[]"]').each(function (i, el) {
    ...
    // ==================== Genre operator ====================
    $('select[name="ctgcon"] option').each(function (i, el) {
        filters.filters.genre_operator.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var genreOpSelected = $('select[name="ctgcon"] option:selected').attr('value') || 'and';
    filters.filters.genre_operator.value = genreOpSelected;
    // ==================== Genres ====================
    $('input[name="categories[]"]').each(function (i, el) {
    ...
    // ==================== Chapters ====================
    $('select[name="totalchapter"] option').each(function (i, el) {
        filters.filters.chapters.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var chaptersSelected = $('select[name="totalchapter"] option:selected').attr('value') || '0';
    filters.filters.chapters.value = chaptersSelected;
    // ==================== Rating operator ====================
    $('select[name="ratcon"] option').each(function (i, el) {
        filters.filters.rating_operator.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var ratconSelected = $('select[name="ratcon"] option:selected').attr('value') || 'min';
    filters.filters.rating_operator.value = ratconSelected;
    // ==================== Rating ====================
    $('select[name="rating"] option').each(function (i, el) {
        filters.filters.rating.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var ratingSelected = $('select[name="rating"] option:selected').attr('value') || '0';
    filters.filters.rating.value = ratingSelected;
    // ==================== Status ====================
    $('select[name="status"] option').each(function (i, el) {
        filters.filters.status.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var statusSelected = $('select[name="status"] option:selected').attr('value') || '-1';
    filters.filters.status.value = statusSelected;
    // ==================== Sort ====================
    $('select[name="sort"] option').each(function (i, el) {
        filters.filters.sort.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var sortSelected = $('select[name="sort"] option:selected').attr('value') || 'rank-top';
    filters.filters.sort.value = sortSelected;
    // ==================== Tag operator ====================
    $('select[name="tagcon"] option').each(function (i, el) {
        filters.filters.tagcon.options.push({
            label: $(el).text().trim(),
            value: $(el).attr('value'),
        });
    });
    var tagconSelected = $('select[name="tagcon"] option:selected').attr('value') || 'and';
    ...
    filters.filters.author.label =
        $('input[name="author"]').prev('label').text().trim() ||
            ...
            'Author';
    // ==================== checks ====================
    if (filters.filters.genres.options.length == 0 ||
    ...
    var json = JSON.stringify(filters, null, 2).replace(/\n(\s*)"options": \[/g, '\n$1// prettier-ignore\n$1"options": [');
    fs.writeFileSync(path.join(__dirname, 'filters', name + '.json'), json + '\n');
...
function getFiltersFromURL(name, url) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch(url + '/search-adv')];
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
