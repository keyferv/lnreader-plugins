...
var generateAll = function () {
    return sources_json_1.default.map(function (source) {
        ...
        if (exist) {
            var filters = (0, fs_1.readFileSync)((0, path_1.join)(folder, 'filters', source.id + '.json'), 'utf-8');
            // strip `// prettier-ignore` comments (JSONC) since JSON.parse doesn't allow them
            source.filters = JSON.parse(filters.replace(/^\s*\/\/[^\n]*$/gm, '')).filters;
        }
        console.log("[novelfire] Generating: ".concat(source.id).concat(' '.repeat(20 - source.id.length), " ").concat(source.filters ? '🔎with filters🔍' : '🚫no filters🚫'));
...
var generator = function generator(source) {
    var _a;
    var NovelFireTemplate = (0, fs_1.readFileSync)((0, path_1.join)(folder, 'template.ts'), {
        encoding: 'utf-8',
    });
    var pluginScript = "\n".concat(NovelFireTemplate, "\nconst plugin = new NovelFirePlugin(").concat(withFilterTypeRefs(JSON.stringify(source)), ");\nexport default plugin;\n    ").trim();
    return {
