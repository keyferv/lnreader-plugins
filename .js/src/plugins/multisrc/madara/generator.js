...
var generateAll = function () {
    return sources_json_1.default.map(function (source) {
        ...
        console.log("[madara] Generating: ".concat(source.id).concat(' '.repeat(20 - source.id.length), " ").concat(source.filters ? '🔎with filters🔍' : '🚫no filters🚫'));
...
var generator = function generator(source) {
    var _a, _b;
    var madaraTemplate = (0, fs_1.readFileSync)((0, path_1.join)(folder, 'template.ts'), {
        encoding: 'utf-8',
    });
    var pluginScript = "\n".concat(madaraTemplate.replace('// CustomJS HERE', ((_a = source.options) === null || _a === void 0 ? void 0 : _a.customJs) || ''), "\nconst plugin = new MadaraPlugin(").concat(JSON.stringify(source), ");\nexport default plugin;\n    ").trim();
    return {
