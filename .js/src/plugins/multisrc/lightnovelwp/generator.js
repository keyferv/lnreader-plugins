...
var generateAll = function () {
    return sources_json_1.default.map(function (source) {
        ...
        console.log("[lightnovelwp] Generating: ".concat(source.id).concat(' '.repeat(20 - source.id.length), " ").concat(source.filters ? '🔎with filters🔍' : '🚫no filters🚫'));
...
var generator = function generator(source) {
    ...
    var pluginScript = "\n".concat(LightNovelWPTemplate.replace('// CustomJS HERE', ((_a = source.options) === null || _a === void 0 ? void 0 : _a.customJs) || ''), "\nconst plugin = new LightNovelWPPlugin(").concat(JSON.stringify(source), ");\nexport default plugin;\n    ").trim();
    return {
