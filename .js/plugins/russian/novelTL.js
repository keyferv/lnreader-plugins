...
var TL = /** @class */ (function () {
    function TL() {
        ...
        this.resolveUrl = function (path) { return _this.site + '/r/' + path; };
        this.filters = {
            tags: {
                label: 'Тэги',
                value: [],
    ...
    TL.prototype.fetchNovels = function (page_1, _a, searchTerm_1) {
                ...
                switch (_j.label) {
                            ...
                            body: JSON.stringify({
                                ...
                                variables: {
                                    filter: {
                                        ...
                                        tags: ((_d = (_c = filters.tags) === null || _c === void 0 ? void 0 : _c.value) === null || _d === void 0 ? void 0 : _d.length)
                                            ? filters.tags.value
                                            : undefined,
                                        genres: ((_f = (_e = filters.genres) === null || _e === void 0 ? void 0 : _e.value) === null || _f === void 0 ? void 0 : _f.length)
                    ...
                    case 1:
                        ...
                        (_h = (_g = data === null || data === void 0 ? void 0 : data.projects) === null || _g === void 0 ? void 0 : _g.content) === null || _h === void 0 ? void 0 : _h.forEach(function (novel) {
                            var _a, _b;
                            var cleanSlug = novel.fullUrl.split('/').pop() || novel.fullUrl;
                            novels.push({
                                name: novel.title,
                                path: cleanSlug,
    ...
    TL.prototype.parseNovel = function (novelPath) {
        return __awaiter(this, void 0, void 0, function () {
            var slug, url, response, body, scriptRegex, match, cleanJson, apolloData, db, resolve, projectKey, project, fallback, covers, coverUrl, loadedCheerio, summary, author, persons, authors, genres, novel, chapters, transProjects, projectsList;
            var _a, _b;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        slug = novelPath.split('/').pop() || novelPath;
                        url = this.site + '/r/' + slug;
                        return [4 /*yield*/, (0, fetch_1.fetchApi)(url)];
                    case 1:
                        response = _c.sent();
                        if (response.status === 404)
                            throw new Error('Novela no encontrada (404).');
                        return [4 /*yield*/, response.text()];
                    case 2:
                        body = _c.sent();
                        scriptRegex = /<script id="serverApp-state" type="application\/json">([\s\S]*?)<\/script>/;
                        match = body.match(scriptRegex);
                        if (!match)
                            throw new Error('Error crítico: JSON de Apollo no encontrado.');
                        cleanJson = match[1].replace(/&q;/g, '"');
                        apolloData = JSON.parse(cleanJson);
                        db = apolloData['apollo.state'];
                        resolve = function (ref) {
                            if (!ref)
                                return null;
                            if (Array.isArray(ref))
                                return ref.map(resolve).filter(Boolean);
                        ...
                        projectKey = Object.keys(db).find(function (k) { return k.startsWith('Project:') && !k.includes('.'); });
                        project = db[projectKey || ''];
                        // Fallback por si la key es rara
                        if (!project) {
                            fallback = Object.keys(db).find(function (k) { return db[k].url === slug && db[k].__typename === 'Project'; });
                            if (fallback)
                                project = db[fallback];
                        }
                        if (!project)
                            throw new Error('Datos del proyecto no encontrados.');
                        covers = resolve(project.covers);
                        coverUrl = ((_a = covers === null || covers === void 0 ? void 0 : covers[0]) === null || _a === void 0 ? void 0 : _a.url)
                            ? this.site + covers[0].url
                            : defaultCover_1.defaultCover;
                        loadedCheerio = (0, cheerio_1.load)(body);
                        summary = loadedCheerio('.info__annotation').text().trim() ||
                            ((_b = project.annotation) === null || _b === void 0 ? void 0 : _b.text) ||
                            '';
                        author = '';
                        persons = resolve(project.persons);
                        if (Array.isArray(persons)) {
                            authors = persons.filter(function (p) { return p.role === 'author' || p.role === 'original_story'; });
                            author = authors
                                .map(function (p) { var _a, _b; return ((_a = p.name) === null || _a === void 0 ? void 0 : _a.lastName) || ((_b = p.name) === null || _b === void 0 ? void 0 : _b.firstName); })
                                .join(', ');
                        }
                        genres = [];
                        loadedCheerio('.tags a, .genres a').each(function (_, el) {
                            var genre = loadedCheerio(el).text().trim();
                            if (genre)
                                genres.push(genre);
                        });
                        novel = {
                            ...
                            cover: coverUrl,
                            author: author,
                            status: statusKey[project.translationStatus || 'unknown'] ||
                        ...
                        chapters = [];
                        transProjects = resolve(project.subprojects);
                        ...
                        projectsList = Array.isArray(transProjects)
                            ? transProjects
                            : transProjects
                                ? [transProjects]
                                : [];
                        projectsList.forEach(function (proj) {
                            ...
                            var vols = resolve(proj.volumes);
                            if (vols && !Array.isArray(vols) && vols.content) {
                                vols = resolve(vols.content);
                            }
                            var volumesList = Array.isArray(vols) ? vols : vols ? [vols] : [];
                            volumesList.forEach(function (vol, vIndex) {
                                if (!vol)
                                    return;
                                var volSlug = vol.url;
                                var volName = vol.name || vol.shortName || "Volumen ".concat(vIndex + 1);
                                // Lo mismo para 'chapters'
                                var chaps = resolve(vol.chapters); // Generalmente es array directo, pero por si acaso
                                var chaptersList = Array.isArray(chaps)
                                    ? chaps
                                    : chaps
                                        ? [chaps]
                                        : [];
                                chaptersList.forEach(function (chap) {
                                    if (!chap)
                                        return;
                                    var chapSlug = chap.url;
                                    var chapTitle = chap.title;
                                    if (!chapTitle) {
                                        chapTitle =
                                            chapSlug === 'ill' ? 'Иллюстрации' : "\u0413\u043B\u0430\u0432\u0430 ".concat(chapSlug);
                                    }
                                    var fullPath = "".concat(project.url || slug, "/").concat(volSlug, "/").concat(chapSlug);
                                    // FORMATO PEDIDO: "Volumen X Capítulo Y"
                                    var displayName = "".concat(volName, " ").concat(chapTitle);
                                    chapters.push({
                                        name: displayName, // Nombre combinado
                                        path: fullPath,
                                        releaseTime: chap.publishDate
                                            ? (0, dayjs_1.default)(chap.publishDate).format('LLL')
                                            : undefined,
    ...
    TL.prototype.parseChapter = function (chapterPath) {
        return __awaiter(this, void 0, void 0, function () {
            var url, body, loadedCheerio, chapterText, scriptRegex, match, cleanJson, data, db_1, textKey;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        url = "".concat(this.site, "/r/").concat(chapterPath);
                        return [4 /*yield*/, (0, fetch_1.fetchApi)(url).then(function (res) { return res.text(); })];
                    case 1:
                        body = _a.sent();
                        loadedCheerio = (0, cheerio_1.load)(body);
                        chapterText = loadedCheerio('.chapter-text').html();
                        if (!chapterText) {
                            scriptRegex = /<script id="serverApp-state" type="application\/json">([\s\S]*?)<\/script>/;
                            match = body.match(scriptRegex);
                            if (match) {
                                ...
                                db_1 = data['apollo.state'];
                                textKey = Object.keys(db_1).find(function (k) {
                                    return db_1[k].text &&
                                        typeof db_1[k].text === 'string' &&
                                        db_1[k].__typename === 'Text';
                                });
                                if (textKey) {
                                    chapterText = db_1[textKey].text;
                                }
                            }
                        }
                        if (chapterText) {
                        ...
                        return [2 /*return*/, 'No se pudo cargar el texto del capítulo.'];
                }
            });
        });
    };
