...
export default function NovelsList({
  ...
  const title = isLatest ? 'Latest Novels' : 'Popular Novels';
  const subtitle = isLatest
    ? 'Browse recently added novels'
    : 'Browse popular novels';
  const Icon = isLatest ? Clock : BookOpen;
  ...
  // Reset cuando cambia el modo o plugin
  useEffect(() => {
    setCurrentIndex(0);
    setMaxIndex(0);
    setNovels([]);
  ...
  // Auto-fetch cuando cambian los filtros (solo si ya se había fetched antes)
  useEffect(() => {
    if (currentIndex > 0) {
  ...
  // Determinar si mostrar botón de filtros
  // En modo 'latest' los filtros están deshabilitados por defecto (como en el original)
  // En modo 'popular' están habilitados si el plugin tiene filtros
  const filtersAvailable = !isLatest && plugin?.filters;
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {plugin ? subtitle : 'Select a plugin to browse novels'}
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button
              ...
              disabled={!filtersAvailable || loading}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              {plugin
                ? `Click the "Fetch" button above to load ${isLatest ? 'latest' : 'popular'} novels from this source.`
